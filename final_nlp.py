...
Original file is located at
    https://colab.research.google.com/drive/15JAd45tX5qNsm2sohXK8EyRD_d9LrtFR
"""
...
if 'spacy_model_status' not in st.session_state:
    st.session_state.spacy_model_status = None

# Enhanced spaCy model loading with auto-install
@st.cache_resource
def install_and_load_spacy_model():
    """Install and load spaCy model with automatic fallback"""
    try:
        import spacy
        # Try to load the model first
        nlp = spacy.load("en_core_web_sm")
        st.session_state.spacy_model_status = "loaded"
        return nlp
    except OSError:
        # Model not found, try to install it
        try:
            st.info("📥 Installing spaCy English model... This may take a moment.")
            ...
            st.session_state.spacy_model_status = "installed_and_loaded"
            return nlp

        except Exception as e:
            st.warning(f"⚠️ Could not install full spaCy model: {str(e)}")
            st.info("🔄 Using basic spaCy model as fallback...")

            try:
                import spacy
                # Fallback to blank English model with basic components
                nlp = spacy.blank("en")

                # Add basic components that don't require trained models
                ...
                st.session_state.spacy_model_status = "basic_fallback"
                st.info("✅ Using basic spaCy model (limited functionality)")
                return nlp

            except Exception as fallback_error:
                st.error(f"❌ All spaCy options failed: {str(fallback_error)}")
                st.info("🔄 Switching to regex-based processing...")
                st.session_state.spacy_model_status = "failed"
                return None
...
# Backup extraction method using regex patterns
def extract_enhanced_roles_backup(text):
    """Backup extraction method using regex patterns when spaCy fails"""
    import re
    ...
    for verb in action_verbs:
        if verb in text_lower:
            action_verb = verb.capitalize()
            break

    # Basic subject/object/verb extraction using patterns
    subject = None
    verb = None
    obj = None
    constraint = None
    ...
    # Basic subject extraction (words before action verb)
    if action_verb:
        try:
            # Split by action verb
            action_verb_pattern = r'\b' + action_verb.lower() + r'\b'
            parts = re.split(action_verb_pattern, text.lower(), 1)

            if len(parts) > 1:
                # Subject is likely before action verb
                subject_part = parts[0].strip()
                subject_words = [w for w in subject_part.split() if w.isalpha()][-3:]
                subject = ' '.join(subject_words) if subject_words else None

                # Object/verb is after action verb
                object_part = parts[1].strip()
                # Remove common words and get meaningful content
                object_words = []
                skip_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'be', 'is', 'are', 'was', 'were'}

                for word in object_part.split()[:10]:  # First 10 words
                    cleaned_word = re.sub(r'[^\w\s]', '', word).lower()
                    if cleaned_word and cleaned_word not in skip_words and len(cleaned_word) > 2:
                        object_words.append(word)
                    if len(object_words) >= 5:  # Limit to 5 meaningful words
                        break

                obj = ' '.join(object_words) if object_words else object_part.split()[0] if object_part.split() else None

                # Try to extract verb from the object part
                verb_patterns = [r'\b(provide|control|monitor|maintain|ensure|verify|check|measure|record|display|indicate|activate|operate|function|perform)\b']
                for pattern in verb_patterns:
                    verb_match = re.search(pattern, object_part.lower())
                    if verb_match:
...
def extract_enhanced_roles(text, nlp):
    """Enhanced NLP extraction with fallback to regex method"""
    if nlp is None:
        # Use backup method if spaCy failed to load
        return extract_enhanced_roles_backup(text)
    ...
    try:
        ...
        for token in doc:
            if hasattr(token, 'lower_') and token.lower_ in ['shall', 'must', 'should', 'will']:
        ...
        if action_verb:
            ...
            for v in verbs:
                ...
                if v_final:
                    verb = v_final.lemma_ if hasattr(v_final, 'lemma_') else str(v_final)
                break
        ...
        # If spaCy processing fails, fall back to regex method
        st.warning(f"spaCy processing failed for text, using backup method: {str(e)}")
        return extract_enhanced_roles_backup(text)
...
def process_dataframe(df, api_key, discipline, document_type, progress_callback=None, use_parallel=True):
    ...
    if st.session_state.spacy_model_status == "loaded":
        st.success("✅ Full spaCy model loaded successfully")
    elif st.session_state.spacy_model_status == "installed_and_loaded":
        st.success("✅ spaCy model installed and loaded successfully")
    elif st.session_state.spacy_model_status == "basic_fallback":
        st.warning("⚠️ Using basic spaCy model (limited functionality)")
    elif st.session_state.spacy_model_status == "failed":
        st.warning("⚠️ spaCy model failed, using regex-based processing")
...
def create_comprehensive_pdf_report(df, discipline, document_type, filename):
    ...
    try:
        ...
        # Project info
        spacy_status = st.session_state.spacy_model_status or "unknown"
        project_info = f"""
        ...
        Total Requirements: {len(df)}<br/>
        spaCy Model Status: {spacy_status}
        """
        ...
        • High Clarity: {high_clarity} ({high_clarity/len(df)*100:.1f}%)<br/>
        • Medium Clarity: {medium_clarity} ({medium_clarity/len(df)*100:.1f}%)<br/>
        • Low Clarity: {low_clarity} ({low_clarity/len(df)*100:.1f}%)<br/>
        • Requirements with Constraints: {with_constraints}<br/>
...
# Main Streamlit App
def main():
    st.markdown('<h1 class="main-header">🔬 NLP Requirements Analysis Tool</h1>', unsafe_allow_html=True)

    st.markdown("""
    This tool processes engineering requirements through comprehensive NLP analysis including:
    - **Text Cleaning & Preprocessing**
    - **Subject-Verb-Object Extraction** (with spaCy auto-install)
    - **Component Refinement with ChatGPT (Parallel)**
    - **Constraint Identification (Parallel)**
    - **INCOSE Compliance Assessment (Sequential)**
    """)

    # Show spaCy model status if available
    if st.session_state.spacy_model_status:
        if st.session_state.spacy_model_status == "loaded":
            st.success("🧠 NLP Engine: Full spaCy model loaded")
        elif st.session_state.spacy_model_status == "installed_and_loaded":
            st.success("🧠 NLP Engine: spaCy model auto-installed and loaded")
        elif st.session_state.spacy_model_status == "basic_fallback":
            st.warning("🧠 NLP Engine: Basic spaCy model (limited functionality)")
        elif st.session_state.spacy_model_status == "failed":
            st.info("🧠 NLP Engine: Regex-based processing (spaCy unavailable)")
    ...
    with st.sidebar:
        ...
        if uploaded_file:
            ...
            if error_msg:
                ...
                if detection_info['method'] == 'smart_detection':
                    st.success(f"✅ **Smart Detection:** Found '{detection_info['numbering_col']}' and '{detection_info['text_col']}'")
                elif detection_info['method'] == 'fallback_position':
                    st.warning(f"⚠️ **Fallback Mode:** {detection_info.get('warning', 'Using first two columns')}")
                ...
                with st.expander("📋 View All Available Columns"):
                    ...
                    for i, col in enumerate(detection_info['all_columns'], 1):
                        ...
                        if col == detection_info.get('numbering_col'):
                            status = " ← **Selected as Numbering**"
                        elif col == detection_info.get('text_col'):
                            status = " ← **Selected as Text**"
                        st.write(f"{i}. {col}{status}")

            st.info(f"🔧 **Processing:** Smart column detection from {detection_info['total_columns'] if detection_info else 'unknown'} columns")
        ...
        if uploaded_file:
            ...
            speed_option = st.radio(
                "Processing Mode",
                ["🚀 Hybrid Processing (Recommended)", "🐌 Full Sequential"],
                index=0,
            ...
            if "Hybrid" in speed_option:
                st.success("✅ Hybrid mode: Parallel processing for refinement & constraints, sequential for INCOSE compliance")
            else:
                st.info("ℹ️ Full sequential mode - All processes run sequentially with rate limiting")
        ...
        if uploaded_file and api_key and st.session_state.selected_discipline and st.session_state.selected_document_type:
            ...
            if discipline_ready and document_type_ready:
                ...
                st.warning("⚠️ Please specify discipline and document type if 'Other' is selected")
        ...
        if st.session_state.processing_complete and st.session_state.processed_df is not None:
            ...
            summary_data = {
                ...
                "spacy_model_status": st.session_state.spacy_model_status or "unknown"
            }
    ...
    if uploaded_file is None:
        ...
        st.markdown('<h2 class="section-header">📋 Expected Data Format</h2>', unsafe_allow_html=True)
        ...
        st.markdown("### 🧠 NLP Processing Features")
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("""
            **🔧 Automatic spaCy Setup:**
            - Auto-detects if spaCy model is available
            - Attempts automatic installation if missing
            - Falls back to basic model if installation fails
            - Uses regex processing as final backup
            """)
        with col2:
            st.markdown("""
            **⚡ Processing Modes:**
            - Hybrid: Parallel + Sequential processing
            - Sequential: Full sequential with rate limiting
        ...
        st.info("📋 Please select discipline and document type in the sidebar")

        # Show current selection status
        if st.session_state.selected_discipline:
            if st.session_state.selected_discipline in ["Custom Discipline", ""]:
                st.warning("⚠️ Please specify your custom discipline")
            else:
                st.success(f"✅ Discipline selected: {st.session_state.selected_discipline}")

        if st.session_state.selected_document_type:
            if st.session_state.selected_document_type in ["Custom Document Type", ""]:
                st.warning("⚠️ Please specify your custom document type")
            else:
                st.success(f"✅ Document type selected: {st.session_state.selected_document_type}")
        ...
        try:
            ...
            if detection_info['method'] == 'smart_detection':
                st.success(f"✅ File processed successfully! Smart detection found '{detection_info['numbering_col']}' and '{detection_info['text_col']}' columns from {detection_info['total_columns']} total columns.")
            else:
                st.success(f"✅ File processed successfully! Used fallback method with first two columns from {detection_info['total_columns']} total columns.")
                if detection_info.get('warning'):
                    st.warning(f"⚠️ {detection_info['warning']}")
                ...
                with st.spinner("Processing requirements..."):
                    ...
                    try:
                        ...
                        if processed_df is not None:
                            ...
                            st.success(f"🎉 Processing completed successfully using {processing_mode} processing!")

                            # Show spaCy model status after processing
                            if st.session_state.spacy_model_status:
                                if st.session_state.spacy_model_status == "loaded":
                                    st.info("🧠 Used full spaCy model for NLP processing")
                                elif st.session_state.spacy_model_status == "installed_and_loaded":
                                    st.info("🧠 Auto-installed and used spaCy model for NLP processing")
                                elif st.session_state.spacy_model_status == "basic_fallback":
                                    st.warning("🧠 Used basic spaCy model (limited functionality)")
                                elif st.session_state.spacy_model_status == "failed":
                                    st.warning("🧠 Used regex-based processing (spaCy unavailable)")
                        else:
            ...
            st.info("💡 For CSV files, try different encoding. For Excel files, check file format.")
    ...
    if st.session_state.processing_complete and st.session_state.processed_df is not None:
        ...
        # Results tabs
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["📈 Summary", "🔍 Detailed Results", "📋 Components", "⚖️ Constraints", "✅ INCOSE Compliance"])
        ...
        with tab1:
            ...
            # NLP Engine Status
            st.markdown("### 🧠 NLP Engine Status")
            if st.session_state.spacy_model_status == "loaded":
                st.success("🎯 **Full spaCy Model:** Complete NLP processing with advanced linguistic features")
            elif st.session_state.spacy_model_status == "installed_and_loaded":
                st.success("🎯 **Auto-Installed spaCy:** Model was automatically installed and loaded successfully")
            elif st.session_state.spacy_model_status == "basic_fallback":
                st.warning("🎯 **Basic spaCy Model:** Limited functionality, some advanced features unavailable")
            elif st.session_state.spacy_model_status == "failed":
                st.info("🎯 **Regex Processing:** spaCy unavailable, using pattern-based text analysis")
            else:
                st.info("🎯 **Unknown Status:** NLP engine status not determined")

            # Processing method explanation
            if "Hybrid" in str(st.session_state.speed_option):
                st.success("🔄 **Processing Method:** Hybrid - Parallel processing for refinement & constraints, sequential for INCOSE compliance analysis")
            else:
                st.info("🔄 **Processing Method:** Full Sequential - All analyses performed sequentially")
        ...
        with tab3:
            st.markdown("### 📋 Extracted Components Analysis")

            col1, col2 = st.columns(2)

            with col1:
                st.markdown("#### 🎯 Most Common Subjects")
        ...
        with tab4:
            ...
            st.markdown("#### 🔝 Requirements with Most Constraints")
        ...
        with tab5:
            st.markdown("### ✅ INCOSE Compliance Analysis")
            st.info("🔄 **Note:** INCOSE compliance analysis was performed sequentially for thorough rule evaluation")
